...
#
#
# abnormals <- merge(returns[[1]]$abnomal, returns[[2]]$abnomal, returns[[3]]$abnomal,
#                    returns[[4]]$abnomal, returns[[5]]$abnomal, returns[[6]]$abnomal,
#                    returns[[7]]$abnomal, returns[[8]]$abnomal, returns[[9]]$abnomal,
#                    returns[[10]]$abnomal)
#
# table <-  parametric_tests(abnormal_returns = abnormals, estimation_start = as.Date("2001-03-05"), estimation_end = as.Date("2001-09-07"), event_start = as.Date("2001-09-10"), event_end = as.Date("2001-10-01"), all = T)
# parametric_tests(abnormal_returns = abnormals, estimation_start = as.Date("2001-03-05"), estimation_end = as.Date("2001-09-07"), event_start = as.Date("2001-09-10"), event_end = as.Date("2001-10-01"), all = T)
#
# #-------------------------------------------------------------------------------
# prices_list <- list()
...
#     rates_list[[i]] <- get_rates_from_prices(prices = prices_list[[i]], quote = "Open", compounding = "periodic")
# }
# index_rates <- get_rates_from_prices(prices = index_prices, quote = "Open", compounding = "periodic")
#
...
# rates1 <- get_rates_from_prices(prices = prices1, quote = "Open", compounding = "periodic")
#
#
...
#
# parametric_tests(list_of_returns = list(allianz, axa), event_start = as.Date("2001-09-10"), event_end = as.Date("2001-09-21"), all = T)
#
